...
#else
  counter += Codec_WriteRegister(0x1A, 0x10);
  counter += Codec_WriteRegister(0x1B, 0x10);
#endif
  counter += Codec_WriteRegister(0xD, 0xc0);// Bump HP gain

  /* Configure the I2S peripheral */
  Codec_AudioInterface_Init(AudioFreq);  

  Codec_VolumeCtrl(g_codec_volume);
...
{         
  if (CurrAudioInterface == AUDIO_INTERFACE_I2S)
  {
    ...
    DMA_DoubleBufferModeConfig(AUDIO_MAL_DMA_STREAM, (uint32_t)&g_aud_abuf[1], 
			       DMA_Memory_0);
